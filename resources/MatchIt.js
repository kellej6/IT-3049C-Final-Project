class MatchIt {
    ...
    try {
      ...
      return await fetch(url)
        ...
        .then(data => {
          ...
          this.capital = this.capital.normalize('NFD').replace(/[\u0300-\u036f]/g, '');
          return data.name;
        });
    } catch {
      console.error(error);
  ...
  guess(letter) {
    ...
    try {
      if (letter.length == 0) { throw new Error("Please provide a letter."); }
      else if (!(letter.match(checkCase)) && letter == " ") { throw new Error("Guess should be a letter."); }
      else if (letter.length > 1) { throw new Error("Please provide one letter."); }
      else {
        letter = letter.toLowerCase();

        if (this.guesses.includes(letter)) { throw new Error("Letter already exists, please provide a new one."); }
        else { this.guesses.push(letter); }

        if (this.capital.includes(letter)) { this.checkWin(); }
        else { this.onWrongGuess(); }
      }
    }
    catch (error) {
      alert(error);
  ...
  getWordHolderText() {
    ...
    if (this.wordHolderCounter == 0) { this.previousGuessedWord.push(wordHolder); this.wordHolderCounter++; }

    let previousGuessed = this.previousGuessedWord[this.previousGuessedWord.length - 1].split("");
    let i = 0;

    while (i < this.capital.length) {
      if (previousGuessed[i] == "_" && wordHolder[i].match(checkCase)) { previousGuessed[i] = wordHolder[i]; }
      i++;
    }
  ...
  drawBase() {
    ...
    this.ctx.fillRect(92.5, 290, 175, 10);
  }

  drawMainBeam() {
    this.ctx.fillStyle = 'white';
    this.ctx.fillRect(175, 10, 10, 280);
  }

  drawNoose() {
    this.ctx.fillStyle = 'white';
    this.ctx.fillRect(325, 10, 10, 50);
  }

  drawTop() {
    this.ctx.fillStyle = 'white';
    this.ctx.fillRect(175, 10, 150, 10);
  }

  drawHead() {
    this.ctx.strokeStyle = 'white';
    this.ctx.beginPath();
    this.ctx.arc(330, 100, 40, 0, 2 * Math.PI);
    this.ctx.stroke();
  }
  ...
  drawBody() {
    this.ctx.strokeStyle = 'white';
    this.ctx.beginPath();
    this.ctx.moveTo(330, 140);
    this.ctx.lineTo(330, 270);
    this.ctx.stroke();
  }
  ...
  drawLeftLeg() {
    this.ctx.strokeStyle = 'white';
    this.ctx.beginPath();
    this.ctx.moveTo(330, 270);
    this.ctx.lineTo(250, 390);
    this.ctx.stroke();
  }
